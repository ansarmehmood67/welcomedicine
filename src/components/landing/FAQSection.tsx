...
const faqs = [
  ...
  { q: "Quanto tempo serve per configurare tutto?", a: "2 minuti. La registrazione è guidata, entro 24 ore configuriamo il tuo profilo e riceverai l'invito per una videocall di formazione e configurazione gratuita." },
  { q: "Posso cancellare quando voglio?", a: "Sì, puoi disdire il tuo abbonamento durante il periodo di prova in qualsiasi momento scrivendo a support@welmed.it. Dopo il periodo di prova l'abbonamento può essere disdetto in qualsiasi momento decorsi i primi sei mesi di abbonamento. I tuoi dati restano tuoi e potrai chiedere al nostro team di esportarli." },
  ...
  { q: "Posso monetizzare le mie consulenze online?", a: "Sì, puoi impostare tariffe personalizzate per videovisite e consulti. La piattaforma gestisce automaticamente l'incasso dei pagamenti e la fatturazione ai pazienti, bollo e sistema Tessera Sanitaria. Puoi fatturare mensilmente il valore delle consulenze svolte al nostro poliambulatorio. Tratteniamo solo il 3% per le spese bancarie e amministrative." },
  ...
  { q: "I medici possono avere una segretaria in piattaforma?", a: "Sì, è possibile creare un account segreteria gratuito e dedicato che può gestire messaggi, appuntamenti e smistamento richieste per conto del medico." },
  ...
  { q: "Posso utilizzare Welcomedicine anche dal cellulare?", a: "Assolutamente sì. Welcomedicine è ottimizzata per tutti i dispositivi: desktop, tablet e smartphone. Puoi accedere dal browser o dall'app dedicata WelmedPro." },
];
        ...
        {faqs.map(({ q, a }, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
            <AccordionItem value={`faq-${i}`} className="rounded-xl border border-border bg-card px-5 data-[state=open]:card-elevated">
              <AccordionTrigger className="text-left text-base font-semibold text-foreground hover:no-underline py-4">{q}</AccordionTrigger>
              <AccordionContent className="text-base text-muted-foreground leading-relaxed pb-4 whitespace-pre-wrap font-extralight">{a}</AccordionContent>
            </AccordionItem>
          </motion.div>
        ))}
      </Accordion>
